import { Mail } from 'lucide-react';
import Layout from '../components/Layout';
import image from "../assets/avatar.jpg"
import Image from "../components/Image"

const Home: React.FC = () => {

  return (
    <Layout>
      <div className="max-w-4xl mx-auto">
        <div className="bg-sidebar rounded-xl shadow-sm border border-muted overflow-hidden">
          <div className="md:flex">
            <div className="h-96 md:h-auto w-full md:w-1/3">
              <Image
                className='w-full h-full object-cover object-top md:object-center'
                src={image}
                blurSrc={'data:image/png;base64,'}
                alt="Dr. Doris Edmund Macha"
              />
            </div>
            <div className="md:w-2/3 p-4 sm:p-8 md:p-12 flex flex-col">
              <h1 className="text-2xl md:text-4xl font-bold text-gray-900 dark:text-white mb-4">
                Dr. Doris Edmund Macha
              </h1>
              <div className="flex-grow max-w-none text-gray-700 dark:text-gray-300 leading-relaxed">
                <p>
                  I am an Assistant Lecturer, Department of Applied Economics, University of Dar es salaam.
                </p>
                <p>
                  My areas of study include: Agricultural & Food Economics, Behavioral Economics, Development Economics.
                </p>
                <p className='mt-2'>
                  Education: M.A. in Economics and B.A. (Honors) in Economics and Statistics, University of Dar es Salaam
                </p>
              </div>
              <div className="flex h-fit flex-wrap gap-4 mt-8">
                <a
                  href="mailto:dorisma15@hotmail.com"
                  className="flex items-center space-x-2 px-4 py-2 bg-accent hover:bg-gray-200 dark:hover:bg-zinc-700 rounded-lg transition-colors"
                >
                  <Mail className="h-4 w-4 text-gray-600 dark:text-gray-300" />
                  <span className="text-sm font-medium text-gray-700 dark:text-gray-300">Email</span>
                </a>
                {/* {homeContent.linkedin && (
                  <a
                    href={homeContent.linkedin}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex items-center space-x-2 px-4 py-2 bg-accent hover:bg-gray-200 dark:hover:bg-gray-600 rounded-lg transition-colors"
                  >
                    <Linkedin className="h-4 w-4 text-gray-600 dark:text-gray-300" />
                    <span className="text-sm font-medium text-gray-700 dark:text-gray-300">LinkedIn</span>
                  </a>
                )}
                {homeContent.twitter && (
                  <a
                    href={homeContent.twitter}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex items-center space-x-2 px-4 py-2 bg-accent hover:bg-gray-200 dark:hover:bg-gray-600 rounded-lg transition-colors"
                  >
                    <Twitter className="h-4 w-4 text-gray-600 dark:text-gray-300" />
                    <span className="text-sm font-medium text-gray-700 dark:text-gray-300">Twitter</span>
                  </a>
                )} */}
              </div>
            </div>
          </div>
        </div>
      </div>
    </Layout>
  );
};

export default Home;